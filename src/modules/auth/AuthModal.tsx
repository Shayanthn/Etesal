...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  return (
    <>
      ...
      {!showRecoveryPrompt && (
      <div 
        className="fixed inset-0 z-[60] bg-slate-950/85 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 md:p-6"
        onClick={onClose}
      >
        <div 
          className="relative w-full max-w-[min(100%,26rem)] max-h-[92dvh] rounded-2xl sm:rounded-3xl bg-slate-900 border border-purple-500/30 shadow-2xl shadow-purple-950/50 text-right overflow-hidden flex flex-col my-auto"
        ...
        >
          ...
          <button
            onClick={onClose}
            className="absolute top-2.5 left-2.5 sm:top-4 sm:left-4 p-1.5 sm:p-2 rounded-xl text-slate-400 hover:text-white hover:bg-slate-800 transition-colors z-20 cursor-pointer"
            aria-label="بستن فرم"
          >
            <X className="w-5 h-5" />
          </button>

          {/* Header Area: Mascot & Tabs */}
          <div className="px-4 sm:px-6 pt-3 sm:pt-5 pb-2 relative z-10 shrink-0">
            {/* Mascot */}
            ...
            <div className="flex items-center p-1 rounded-xl sm:rounded-2xl bg-slate-950 border border-slate-800">
              <button
                type="button"
                onClick={() => { setMode('login'); setIsError(false); }}
                className={`flex-1 py-1.5 sm:py-2 px-1 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-black transition-all cursor-pointer text-center whitespace-nowrap ${
                  mode === 'login'
          ...
          <div className="overflow-y-auto overscroll-contain px-4 sm:px-6 pb-4 sm:pb-6 relative z-10 space-y-2.5 sm:space-y-3.5 flex-1">
            <form onSubmit={handleSubmit} className="space-y-2.5 sm:space-y-3.5">
              
              {/* Username */}
              <div className="space-y-1 sm:space-y-1.5">
                <label className="text-[11px] sm:text-xs font-bold text-slate-300 block">نام کاربری (Username)</label>
                <div className="relative">
                  ...
                  <input
                    ...
                    className="w-full pl-4 pr-10 py-2 sm:py-2.5 rounded-xl bg-slate-950/60 border border-slate-800 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-purple-500 transition-colors text-left font-mono"
                    dir="ltr"
                    autoFocus
                  />
                </div>
              ...
              <div className="space-y-1 sm:space-y-1.5">
                <div className="flex items-center justify-between">
                  <label className="text-[11px] sm:text-xs font-bold text-slate-300">کلمه عبور (Password)</label>
                  {mode === 'login' && (
                    <button
                      type="button"
                      onClick={() => triggerError('بازیابی رمز عبور 📩', 'در صورت فراموشی، به پشتیبانی آنلاین پیام دهید یا از ایمیل بازیابی استفاده کنید.')}
                      className="text-[10px] sm:text-[11px] text-purple-400 hover:underline cursor-pointer"
                    >
                      فراموشی رمز؟
                    </button>
                  )}
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    onBlur={handleInputBlur}
                    placeholder="••••••••"
                    className="w-full pl-10 pr-10 py-2 sm:py-2.5 rounded-xl bg-slate-950/60 border border-slate-800 text-xs text-white placeholder:text-slate-500 focus:outline-none focus:border-purple-500 transition-colors text-left font-mono"
                    dir="ltr"
                  />
                  <button
                    type="button"
                ...
                {mode === 'register' && password.length > 0 && (
                  <div className="pt-0.5 space-y-1 animate-fade-in">
              ...
              {mode === 'register' && (
                <div className="space-y-1 sm:space-y-1.5 animate-fade-in">
                  <label className="text-[11px] sm:text-xs font-bold text-slate-300 block">تکرار کلمه عبور</label>
                  <div className="relative">
              ...
              <div className="flex items-center justify-between pt-0.5">
                ...
                <span className="text-[9px] sm:text-[10px] text-emerald-400 flex items-center gap-1 font-mono">
                  <ShieldCheck className="w-3.5 h-3.5" />
                  <span>Zero-Log Encrypted</span>
                </span>
              </div>
              ...
              >
                {isSubmitting ? (
                  ...
                  <>
                    ...
                    <span>ثبت‌نام آنی و دریافت هدیه</span>
                  </>
                )}
              </button>
              ...
              <div className="relative flex items-center justify-center my-2 sm:my-3">
                ...
                <div className="relative bg-slate-900 px-3 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                  یا از طریق
                </div>
              </div>
              ...
              <button
                ...
                disabled={isSubmitting}
                className="w-full py-2 sm:py-2.5 px-4 rounded-xl sm:rounded-2xl bg-white hover:bg-slate-100 text-slate-900 font-bold text-xs sm:text-sm shadow-lg transition-all flex items-center justify-center gap-3 cursor-pointer disabled:opacity-50"
              >
                ...
                <span>{mode === 'login' ? 'ورود با حساب گوگل' : 'ثبت‌نام سریع با گوگل'}</span>
              </button>
            </form>
          </div>
