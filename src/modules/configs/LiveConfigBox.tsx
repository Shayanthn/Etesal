      ...
      <div className="rounded-3xl bg-slate-900/80 border border-slate-800 p-6 shadow-2xl backdrop-blur-xl">
        ...
        {filteredConfigs.length === 0 ? (
          <div className="py-12 px-6 text-center rounded-2xl bg-slate-800/20 border border-dashed border-slate-700/60 my-6 space-y-3">
            ...
            <div className="text-sm font-bold text-slate-300">کانفیگ فعالی برای این اپراتور یافت نشد</div>
            <p className="text-xs text-slate-500 max-w-md mx-auto">
              کانفیگ‌ها به صورت خودکار توسط ورکر اعتبارسنجی و تست پینگ می‌شوند. دکمه «تست مجدد پینگ» را جهت پایش مجدد بزنید یا تب دیگری را انتخاب کنید.
            </p>
            <button
              onClick={onRefreshPing}
              disabled={isTestingPing}
              className="mt-2 inline-flex items-center gap-1.5 px-4 py-2 rounded-xl bg-purple-600/30 hover:bg-purple-600/50 text-purple-200 text-xs font-bold border border-purple-500/40 cursor-pointer"
            >
              <RefreshCw className={`w-3.5 h-3.5 ${isTestingPing ? 'animate-spin text-purple-400' : ''}`} />
              <span>پایش و دریافت مجدد کانفیگ‌ها</span>
            </button>
          </div>
        ) : (
